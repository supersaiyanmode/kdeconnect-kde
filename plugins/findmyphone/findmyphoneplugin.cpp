...
K_PLUGIN_FACTORY( KdeConnectPluginFactory, registerPlugin< FindMyPhonePlugin >(); )
K_EXPORT_PLUGIN( KdeConnectPluginFactory("kdeconnect_findmyphone", "kdeconnect-plugins") )

FindMyPhonePlugin::FindMyPhonePlugin(QObject* parent, const QVariantList& args)
    : KdeConnectPlugin(parent, args)
{
    //kDebug(debugArea()) << "FindMyPhone plugin constructor for device" << device()->name();
}

FindMyPhonePlugin::~FindMyPhonePlugin()
{
    //kDebug(debugArea()) << "FindMyPhone plugin destructor for device" << device()->name();
}
...
void FindMyPhonePlugin::sendPing()
{
...
void FindMyPhonePlugin::sendPing(const QString& customMessage)
{
    NetworkPackage np(PACKAGE_TYPE_FINDMYPHONE);
